...
def train_model(
    ...
    for epoch in range(num_epochs):
        train_preds, train_labels, train_loss, val_preds, val_labels, val_loss = (
            [],
            [],
            0,
            [],
...
def ndgcn(evidence, scores, n=None, idcg_1=False):
    ...
    res = (sorted_evidence / log_indices).sum() / (
        ideal_sorted_evidence / log_indices
    ).sum()
    return res
        ...
        for eval_class in eval_classes:
            ...
            if 0 < evidence_pos.sum() < len(evidence_pos):
                ...
                auprc = average_precision_score(
                    evidence_pos.detach().cpu().numpy(), patch_scores
                )
                ...
                scores["ndgcn"].append(
                    ndgcn(evidence_rank.detach().cpu().numpy(), patch_scores, ndgcn_n)
                )
                scores["pearsonr"].append(
                    pearsonr(evidence.detach().cpu().numpy(), patch_scores)
                )
                scores["spearmanr"].append(
                    spearmanr(evidence.detach().cpu().numpy(), patch_scores)
                )

            if 0 < evidence_neg.sum() < len(evidence_neg):
                auroc_2.append(
                    roc_auc_score(evidence_neg.detach().cpu().numpy(), -patch_scores)
                )
                auprc_2.append(
                    average_precision_score(
                        evidence_neg.detach().cpu().numpy(), -patch_scores
                    )
                )
