...
def get_args():
    ...
    parser.add_argument(
        "--baseline", action="store_true", help="Whether to compute the random baseline"
    )
    parser.add_argument(
        "--morl-abs",
        action="store_true",
...
def main():
    ...
    for heatmap_type in args_user.explanation_types:
        ...
        if (
            args_user.explain_scores_path is not None
            and heatmap_type in args_user.precomputed_heatmap_types
        ):
        ...
        if args_user.morl_abs:
            ...
            df_results_morl_abs = xmodel_eval.patch_drop_or_add(
                data_loader,
                attribution_strategy="abs",
                order="morl",
                approach=args_user.approach,
                strategy=args_user.strategy,
                max_bag_size=args_user.max_bag_size,
    ...
    if args_user.baseline:
        ...
        df_results_random = xmodel_eval.patch_drop_or_add(
            data_loader,
            attribution_strategy="random",
            order="morf",
            approach=args_user.approach,
            strategy=args_user.strategy,
            max_bag_size=args_user.max_bag_size,
