...
def reorder(values, col_indices, row_indices):
    col_major_indices = col_indices * 1000000 + row_indices
    perm = col_major_indices.argsort()

    batch_size, seq_len = perm.shape
    batch_indices = (
        torch.arange(batch_size, device=values.device).view(-1, 1).repeat(1, seq_len)
    )

    return values[batch_indices, :, perm].transpose(-1, -2)


def reverse_reorder(values, col_indices, row_indices):
    col_major_indices = col_indices * 1000000 + row_indices
    inverse_perm = col_major_indices.argsort().argsort()

    batch_size, seq_len = inverse_perm.shape
    batch_indices = (
        torch.arange(batch_size, device=values.device).view(-1, 1).repeat(1, seq_len)
    )

    return values[batch_indices, :, inverse_perm].transpose(-1, -2)
...
class CrossMamba(nn.Module):
    def __init__(
        ...
        if dt_init == "constant":
            ...
            nn.init.constant_(self.dt_proj_c.weight, dt_init_std)
            nn.init.constant_(self.dt_proj_d.weight, dt_init_std)
        elif dt_init == "random":
            ...
            nn.init.uniform_(self.dt_proj_c.weight, -dt_init_std, dt_init_std)
            nn.init.uniform_(self.dt_proj_d.weight, -dt_init_std, dt_init_std)
        else:
        ...
        with torch.no_grad():
            ...
            self.dt_proj_d.bias.copy_(inv_dt)
        ...
        self.dt_proj_d.bias._no_reinit = True
    ...
    def forward(self, hidden_states, positions_x, positions_y):
        """
        hidden_states: (B, L, D)
        ...
        A_d = -torch.exp(self.A_d_log.float())  # (d_inner, d_state)
        ...
        xz_c = reorder(values=xz, col_indices=positions_x, row_indices=positions_y)
        x_c, z_c = xz_c.chunk(2, dim=1)

        x_c = self.conv1d_c(x_c)[..., :seqlen]
        x_c = self.act(x_c)

        x_c_dbl = self.x_proj_c(rearrange(x_c, "b d l -> (b l) d"))  # (bl d)
        dt_c, B_c, C_c = torch.split(
            x_c_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1
        )
        ...
        y_c = y_c * self.act(z_c)

        y_c = reverse_reorder(
            values=y_c, col_indices=positions_x, row_indices=positions_y
        )

        # d

        xz_d = reorder(xz.flip([-1]), col_indices=positions_x, row_indices=positions_y)
        x_d, z_d = xz_d.chunk(2, dim=1)

        x_d = self.conv1d_d(x_d)[..., :seqlen]
        x_d = self.act(x_d)

        x_d_dbl = self.x_proj_d(rearrange(x_d, "b d l -> (b l) d"))  # (bl d)
        dt_d, B_d, C_d = torch.split(
            x_d_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1
        )
        ...
        y_d = y_d * self.act(z_d)

        y_d = reverse_reorder(
            y_d.flip([-1]), col_indices=positions_x, row_indices=positions_y
        )
