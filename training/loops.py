    ...
    for i_epoch in tqdm(range(n_epochs)):
        ...
        if not i_epoch % n_epoch_val:  # validation for every {n_epoch_val} epochs
            ...
            for i_batch, batch in enumerate(dataloader_val):
                ...
                prob_pred_val, labels_val, loss, _ = classifier.validation_step(
                    batch, softmax=True
                )
...
def test_classification_model(
    ...
    for i_batch, batch in enumerate(tqdm(dataloader_test)):
        ...
        prob_pred_val, labels_val, loss, pred_metadata = classifier.validation_step(
            batch, softmax=True
        )
        ...
        if isinstance(pred_metadata["slide_id"], dict):
            all_sample_ids = {
                key: all_sample_ids.get(key, []) + ids
                for key, ids in pred_metadata["slide_id"].items()
            }
        else:
            all_sample_ids = {
                "slide_id": all_sample_ids.get("slide_id", [])
                + pred_metadata["slide_id"]
            }
    ...
    results = callback.save_test_results(
        auc_test,
        loss_test,
        all_preds,
        all_labels,
